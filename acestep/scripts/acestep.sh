...
# Output dir at same level as .claude/.codex: walk up 4 levels from scripts/
# (a loop instead of a literal path-traversal expression)
_acestep_root="$SCRIPT_DIR"
for _ in 1 2 3 4; do _acestep_root="$(dirname "$_acestep_root")"; done
OUTPUT_DIR="${_acestep_root}/acestep_output"
...
ensure_connection() {
    ...
    echo -e "${YELLOW}Cannot connect to: $endpoint${NC}" >&2
...
cmd_models() {
    check_deps
    local endpoint=$(ensure_connection)
    local bearer=$(load_api_key)

    echo "Available Models:"
    echo "----------------------------------------"
    if [ -n "$bearer" ]; then
        curl -s -H "Authorization: Bearer ${bearer}" "${endpoint}/v1/models"
    else
...
query_job_result() {
    ...
    if [ -n "$bearer" ]; then
        curl -s -X POST "${endpoint}/query_result" \
...
cmd_status() {
    ...
    local endpoint=$(ensure_connection)
    local response=$(query_job_result "$endpoint" "$job_id")
    ...
    case "$status" in
        ...
        1)
            ...
            download_audios "$endpoint" "$job_id" "$result_file"
            rm -f "$result_file"
            ;;
        2)
            echo "Status: failed"
...
download_audios() {
    ...
    while IFS= read -r audio_path; do
        ...
        if [ -n "$audio_path" ]; then
            ...
            local download_url="${endpoint}${audio_path}"
...
get_completion_model() {
    ...
    local response
    if [ -n "$bearer" ]; then
        response=$(curl -s -H "Authorization: Bearer ${bearer}" "${endpoint}/v1/models" 2>/dev/null)
    else
        response=$(curl -s "${endpoint}/v1/models" 2>/dev/null)
    fi
...
cmd_generate() {
    ...
    if [ "$api_mode" = "completion" ]; then
        ...
        local model_id=$(get_completion_model "$endpoint" "$model")
...
" "$src_audio" > "$audio_b64_file"
        ...
        send_completion_request "$endpoint" "$temp_payload"
    else
        ...
        local response
        if [ -n "$bearer" ]; then
            response=$(curl -s -X POST "${endpoint}/release_task" \
                -H "Content-Type: application/json; charset=utf-8" \
                -H "Authorization: Bearer ${bearer}" \
...
cmd_random() {
    ...
    if [ "$api_mode" = "completion" ]; then
        # --- Completion mode ---
        local model_id=$(get_completion_model "$endpoint" "")
        local def_audio_format=$(get_config "generation.audio_format")
